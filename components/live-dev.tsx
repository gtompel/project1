        ...
        <div className={inline ? "fixed right-4 top-16 z-[60] w-[340px] max-w-[90vw]" : "fixed right-4 bottom-16 z-[60] w-[340px] max-w-[90vw]"}>
          <Card className="p-4 space-y-3 text-sm">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setOpen(false)}>Закрыть</Button>
            </div>
            ...
            <div className="pt-2">
              <div className="text-xs uppercase tracking-wide text-muted-foreground mb-1">Performance</div>
              <div className="grid grid-cols-2 gap-2">
                <span className="text-muted-foreground">TTFB</span><span>{perf.ttfb ? `${Math.round(perf.ttfb)} ms` : '—'}</span>
                <span className="text-muted-foreground">FCP</span><span>{perf.fcp ? `${Math.round(perf.fcp)} ms` : '—'}</span>
                <span className="text-muted-foreground">DCL</span><span>{perf.domContentLoaded ? `${Math.round(perf.domContentLoaded)} ms` : '—'}</span>
                <span className="text-muted-foreground">Load</span><span>{perf.load ? `${Math.round(perf.load)} ms` : '—'}</span>
              </div>
            </div>
            <div className="pt-2">
